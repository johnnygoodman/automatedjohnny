...
Gmail.new( GMAIL['username'], GMAIL['password']) do |gmail|
  ...
  inbox.emails(:unread).each do |email|
    ...
    if email.subject =~ /\[Invoice\] Basecamp Monthly Subscription #[0-9]{7}/ || email.subject =~ /Fwd: \[Invoice\] Basecamp Monthly Subscription #[0-9]{7}/
      
      #this is a perfect place for learning more about the Enumeration ruby class.
      ...
      aj.thirtysevensignals("Web Software Rental", amount, "37Signals.com - Basecamp Project Management", conf, '', "CPAP.com", date)  
    else
